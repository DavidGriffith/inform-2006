...
[ SubjectNotPlayer obj reportage v2 v3;
    if (reportage && actor ~= player) {
        ...
        if (obj == actor)           print (theActor) obj, " ", (string) v3, " ";
        else
            if (obj has pluralname) print (the) obj, " ", (string) v2, " ";
            else                    print (the) obj, " ", (string) v3, " ";
    }
    else
        if (obj has pluralname)     print (The) obj, " ", (string) v2, " ";
        else                        print (The) obj, " ", (string) v3, " ";
...
[ CSubjectCan_ obj reportage; CSubjectVerb(obj,reportage,"can",0,"can"); ];

[ CSubjectCant obj reportage; CSubjectVerb(obj,reportage,"can't",0,"can't"); ];

[ CSubjectDont obj reportage; CSubjectVerb(obj,reportage,"don't",0,"doesn't"); ];
...
[ DecideAgainst; CSubjectVerb(actor,false,"decide",0,"decides"); "that's not such a good idea."; ];
...
! Ask:      see Answer
  ...
  Blow:     CSubjectCant(actor,true); "usefully blow ", (ThatOrThose) x1, ".";
  Burn: switch (n) {
  ...
  Consult:  CSubjectVerb(actor,true,"discover",0,"discovers"); "nothing of interest in ", (the) x1, ".";
  Cut: switch (n) {
  ...
  Disrobe: switch (n) {
        ...
        2:  CSubjectVerb(actor,false,"take off",0,"takes off"); "", (the) x1, ".";
    }
  Drink:    "There's nothing suitable to drink here.";
  Drop: switch (n) {
        1:  CSubjectIs__(x1,true); "already here.";
        2:  CSubjectHas_(actor,true); "not got ", (ThatOrThose) x1, ".";
  ...
  Enter: switch (n) {
        1:  CSubjectIs__(actor,true); "already ", (nop) SupportObj(x1,"on","in"), (the) x1, ".";
        2:  CSubjectIs__(x1,true); print "not something ", (theActor) actor, " can ";
                switch (x2) {
                  'stand':  "stand on.";
                  'sit':    "sit down on.";
                  'lie':    "lie down on.";
        ...
        3:  CSubjectCant(actor,true); "get into the closed ", (name) x1, ".";
        4:  CSubjectCan_(actor,true); "only get into something free-standing.";
        5:  CSubjectVerb(actor,false,"get",0,"gets"); SupportObj(x1,"onto","into"); "", (the) x1, ".";
  ...
  Examine: switch (n) {
        ...
        2:  CSubjectVerb(actor,true,"see",0,"sees"); "nothing special about ", (the) x1, ".";
        3:  CSubjectIs__(x1,true); print "currently switched ";
                if (x1 has on) "on."; else "off.";
    }
  Exit: switch (n) {
        1:  CSubjectIsnt(actor,true); "in anything at the moment.";
        2:  CSubjectCant(actor,false); "get out of the closed ", (name) x1, ".";
        3:  CSubjectVerb(actor,false,"get",0,"gets"); SupportObj(x1,"off","out of"); "", (the) x1, ".";
        4:  CSubjectIsnt(actor,true); SupportObj(x1,"on","in"); "", (the) x1, ".";
        5:  "(first getting ", (nop) SupportObj(x1,"off","out of"), (the) x1, ")";
  ...
  Pray:     "Nothing practical results from this prayer.";
  Prompt:   print "^>";
  Pronouns: switch (n) {
  ...
  Pull,Push,Turn: switch (n) {
        1:  CSubjectIs__(x1,true); "fixed in place.";
        2:  CSubjectIs__(actor,true); "unable to.";
...
! Push: see Pull
  ...
  Sing:     CSubjectVerb(actor,false,"sing",0,"sings"); "abominably.";
  ...
  Tell: switch (n) {
        1:  CSubjectVerb(actor,false,"conduct",0,"conducts"); "a brief monologue.";
