...
[ VersionSub ix;
    ...
    #Ifnot; ! TARGET_GLULX;
    @gestalt 1 0 ix;
    print "Interpreter version ", ix / $10000, ".", (ix & $FF00) / $100, ".", ix & $FF, " / ";
    @gestalt 0 0 ix;
...
[ NextEntry o odepth;
    for (::) {
        ...
        if (c_style & CONCEAL_BIT && (o has concealed or scenery)) continue;
        return o;
    }
];
...
[ WriteListR o depth stack_pointer  classes_p sizes_p i j k k2 l m n q senc mr;
    ...
    for (::) {
        ...
        if (c_style & CONCEAL_BIT && (o has concealed or scenery)) {
            o = sibling(o);
            continue;
        }
        break;
...
[ WriteBeforeEntry o depth ipos sentencepos
    ...
    inventory_stage = 1;
    if (c_style & INDENT_BIT) Print__Spaces(2*(depth+wlf_indent));
    if (o.invent && (c_style & (FULLINV_BIT+PARTINV_BIT))) {
...
! only if they are accessible to the player, rtn might end with these statements:
!   if ((o has transparent or supporter) || (o has container && o has open)) rtrue;
!   rfalse;
...
[ FullScoreSub i;
    ...
    for (i=0 : i<NUMBER_TASKS : i++)
        if (task_done->i == 1) {
            PANum(TaskScore(i));
            if (PrintTaskName(i) == false) LibraryExtensions.RunAll(ext_printtaskname, i);
        }
    if (things_score) {
        PANum(things_score);
        L__M(##FullScore, 2);
...
[ AttemptToTakeObject item
    ...
    if (item == actor)    return L__M(##Take, 2, item);
    ...
    ! Can the actor touch the item, or is there (e.g.) a closed container in the way?
    ...
    ! The item is now known to be available for taking.  Is the player carrying
    ! too much?  If so, possibly juggle items into the rucksack to make room.
...
[ InsertSub ancestor;
    ...
    if (second ~= ancestor) {
        action = ##Receive;
        if (RunRoutines(second, before)) { action = ##Insert; return; }
        action = ##Insert;
        if (second has container && second hasnt open && ImplicitOpen(second)) return L__M(##Insert, 3, second);
    }
    ...
    if (second ~= ancestor) {
        action = ##Receive;
        if (RunRoutines(second, after)) { action = ##Insert; return; }
        action = ##Insert;
    }
    if (keep_silent) return;
...
[ EmptyTSub i j k flag;
    ...
    if (second ~= d_obj) {
        if (second hasnt supporter) {
            ...
            if (second hasnt open && ImplicitOpen(second)) return L__M(##EmptyT, 2, second);
        }
    }
...
[ ExitSub p;
    ...
    if (p has container && p hasnt open && ImplicitOpen(p)) return L__M(##Exit, 2, p);
    if (noun == nothing) {
        inp1 = p;
    ...
    if (actor provides posture) actor.posture = 0;
        ...
        if (o hasnt concealed && NotSupportingThePlayer(o)) {
            ...
            if (o hasnt scenery) {
              ...
              .Prop_Chosen;
                ...
                if (o hasnt moved || o.&describe || f2 == 1) {
...
[ OpenSub;
    ...
    if (noun has container && noun hasnt transparent && location ~= thedark
        && VisibleContents(noun) && IndirectlyContains(noun, actor) == 0)
