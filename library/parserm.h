...
[ Keyboard  a_buffer a_table  nw i w w2 x1 x2;
  ...
  .FreshInput;
    ...
    a_table->0  = MAX_BUFFER_WORDS;
    #Endif; ! TARGET_

    ! Print the prompt, and read in the words and dictionary addresses

    L__M(##Prompt);
    if (AfterPrompt() == false) LibraryExtensions.RunAll(ext_afterprompt);
    #IfV5;
    DrawStatusLine();
    #Endif; ! V5
    KeyboardPrimitive(a_buffer, a_table);
                ...
                if (line_tdata-->pcount == MULTIEXCEPT_TOKEN or MULTIINSIDE_TOKEN  && i == 1) {
                    ...
                    if (line_ttype-->pcount == PREPOSITION_TT) {
                        ...
                        if ((line_ttype-->pcount == ELEMENTARY_TT) && (line_tdata-->pcount == NOUN_TOKEN)) {
                            ...
                            while (wn < num_words) {
                                l=NextWord();
                                if (l && (l->#dict_par1) & DICT_PREP) {   ! if preposition
...
[ NounDomain domain1 domain2 context    first_word i j k l
  ...
  .Incomplete;
    ...
    ! Once again, if the reply looks like a command (that is, VERB ... or XXX,VERB ...),
    ! give it to the parser to get on with and forget about the question...
...
[ TryGivenObject obj threshold k w j;
    ...
    if (threshold == -1) {
        ...
        LibraryExtensions.BetweenCalls = LibraryExtensions.RestoreWN; ! restore wn each pass
...
Object  InformLibrary "(Inform Library)"
  with  play [ i j k l;
            ...
            if (deadflag ~= 2 && AfterLife() == false) LibraryExtensions.RunAll(ext_afterlife);
        ...
        end_turn_sequence [;
            ...
            if (TimePasses() == false) LibraryExtensions.RunAll(ext_timepasses);
                                        if (deadflag) return;
...
[ GameEpilogue;
    ...
    switch (deadflag) {
      ...
      2:        L__M(##Miscellany, 4);
      default:  print " ";
                if (DeathMessage() == false) LibraryExtensions.RunAll(ext_deathmessage);
                print " ";
    }
    print "***";
    #Ifdef TARGET_ZCODE;
...
[ BeforeRoutines rv;
    rv = GamePreRoutine();
    if (rv == false) rv = LibraryExtensions.RunWhile(ext_gamepreroutine, false);
    if (rv) rtrue;
...
[ Box__Routine maxwid arr ix lines lastnl parwin;
    ...
    if (gg_quotewin == 0) {
        ...
        if (ix == false) ix = LibraryExtensions.RunWhile(ext_InitGlkWindow, false, GG_QUOTEWIN_ROCK);
        if (ix == false)
...
[ GGInitialise res;
    ...
    res = InitGlkWindow(0);
    if (res == false) res = LibraryExtensions.RunWhile(ext_InitGlkWindow, false, 0);
    if (res) return;
    ...
    if (gg_mainwin == 0) {
        ...
        if (res == false) res = LibraryExtensions.RunWhile(ext_InitGlkWindow, false, GG_MAINWIN_ROCK);
        if (res == false)
            gg_mainwin = glk($0023, 0, 0, 0, 3, GG_MAINWIN_ROCK); ! window_open
        if (gg_mainwin == 0) {
    ...
    if (gg_statuswin == 0) {
        ...
        if (res == false) res = LibraryExtensions.RunWhile(ext_InitGlkWindow, false, GG_STATUSWIN_ROCK);
        if (res == false) {
    ...
    if (InitGlkWindow(1) == false) LibraryExtensions.RunWhile(ext_InitGlkWindow, false, 1);
];
...
[ GGRecoverObjects id;
    ...
    if (IdentifyGlkObject(0) == false) LibraryExtensions.RunWhile(ext_identifyglkobject, false, 0);
    ...
    if (IdentifyGlkObject(2) == false) LibraryExtensions.RunWhile(ext_identifyglkobject, false, 2);
];
...
[ StringSize a b c d e;
    PrintToBuffer(StorageForShortName, SHORTNAMEBUF_LEN, a, b, c, d, e);
...
[ PrefaceByArticle o acode pluralise capitalise  i artform findout artval;
    ...
    if (standard_interpreter && findout) {
        ...
        findout = 0;                        ! To find the first char of the object's
                                            ! short-name, we need (worst case) to
                                            ! unpack the first two words
...
[ CIndefArt o
    ...
    if (o has proper) {
        ...
        if (o provides article) {
            PrintCapitalised(o, article, true); print " ", (PSN__) o;
        }
        else
...
Object  LibraryExtensions "(Library Extensions)"
  with  RunAll [ prop a1 a2 a3
        ...
        BetweenCalls 0, ! can be set to a function (e.g. RestoreWN) meant for execution after non-terminating calls to extension objects (via RunUntil/While/All)
